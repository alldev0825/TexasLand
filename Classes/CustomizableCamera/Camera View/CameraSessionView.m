...
-(void)InitialSetups
{
    ...
    [self setupCaptureManager:RearFacingCamera];
    cameraBeingUsed = RearFacingCamera;
    // [self composeInterface];//[self onTapDismissButton]//[self onTapToggleButton]
    [self.captureBtn addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
    [self.crossBtn addTarget:self action:@selector(onTapDismissButton) forControlEvents:UIControlEventTouchUpInside];
    [self.toggleBtn addTarget:self action:@selector(onTapToggleButton) forControlEvents:UIControlEventTouchUpInside];
    [[_captureManager captureSession] startRunning];
...
-(void)setupCaptureManager:(CameraType)camera {
    ...
    if (_captureManager) {
        ...
        //Preview Layer setup
        NSLog(@"width is %f",self.middleView.frame.size.width);
        self.heightC.constant = self.middleView.frame.size.width;
        
        CGRect layerRect = self.middleView.layer.bounds;
        ...
        [self.middleView.layer addSublayer:_captureManager.previewLayer];
...
-(void)composeInterface {
    ...
    if (_topBarView) {
        ...
        _bottomBarView.frame  = (CGRect){0,self.frame.size.height-200,self.frame.size.width,200};
        ...
        if (_cameraToggle) {
          //  _cameraToggle.frame = (CGRect){self.frame.size.width-barButtonItemSize.width-30,self.frame.size.height-barButtonItemSize.height-30, barButtonItemSize};
          //  _cameraToggle.center = CGPointMake(_topBarView.center.x * 1.20, _topBarView.center.y);
            _cameraToggle.frame = (CGRect){self.frame.size.width-barButtonItemSize.width-10,0, barButtonItemSize};
            _cameraToggle.center = CGPointMake(self.frame.size.width-barButtonItemSize.width-10, _bottomBarView.center.y);
        ...
        if (_cameraDismiss) {
            _cameraDismiss.frame = (CGRect){self.frame.size.width-barButtonItemSize.width-10,0, barButtonItemSize};
            _cameraDismiss.center = CGPointMake(self.frame.size.width-barButtonItemSize.width-10, _topBarView.center.y);
...
//    exit(0);
    ...
    if (self.delegate)
    {
        if ([self.delegate respondsToSelector:@selector(cancelCaptureImage:)])
            [self.delegate cancelCaptureImage:[[self captureManager] stillImage]];
    }
}
